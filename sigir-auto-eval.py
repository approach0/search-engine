...
def do_evaluation(run_file, medium, index, i):
    # Run make
    ...
    time_file = run_file + ('.runtime.%d.dat' % i)
...
for i in range(repeats):
    with open(input_tsv) as fd:
        ...
        for idx, row in enumerate(rd):
            ...
            do_evaluation('./tmp/' + run_name, medium, index, i)

# get efficiency stats in all repeated runs
with open(input_tsv) as fd:
    rd = csv.reader(fd, delimiter="\t")
    for idx, row in enumerate(rd):
        if idx == 0: continue # skip the header row
        run_name = row[0]
        path = './tmp/%s.runtime.%d.dat'
        time_files = [path % (run_name, i) for i in range(repeats)]
        command = ['python3', './stats-efficiency-summary.py'] + time_files
        print(command)
        fh = open('./tmp/' + run_name + '.stats.dat', 'w')
        res = subprocess.run(command, stdout=fh)
        fh.close()
