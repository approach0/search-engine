...
void BM25_score_args_print(struct BM25_score_args *args, int n)
{
	uint32_t i;
...
float BM25_tf_partial_score(struct BM25_score_args* args,
	uint32_t i, float tf, float doclen)
{
...
float BM25_idf(struct BM25_score_args* args, float df)
{
	return logf((args->docN - df + 0.5f) / (df + 0.5f));
}
