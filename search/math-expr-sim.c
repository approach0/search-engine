...
math_expr_prefix_highlight_on_merge(
	uint64_t cur_min, struct postmerge *pm,
	struct math_extra_score_arg *mesa,
	struct indices *indices, FILE *fh
)
{
	struct math_postlist_item *item = NULL;
	...
	if (item) {
		size_t txt_sz;
		char *txt = get_blob_string(indices->txt_bi, item->doc_id, 1, &txt_sz);
		char *tex = get_expr_by_pos(txt, txt_sz, item->exp_id);
		free(txt);
		{
			const char graph_output_file[] = "./graph.tmp.txt";
			FILE *fh = fopen(graph_output_file, "w");
			printf("tree highlight: %s\n", tex);
			(void)math_tree_highlight(tex, dmap, MAX_MTREE, fh);
			fclose(fh);
		}
	}
