function db_push_query(db, qry_json) {
...
function db_pull_query_items(db, max, date_range) {
...
function db_pull_query_IPs(db, max, date_range) {
	if (date_range === undefined) date_range = {};
	const date_begin = date_range['begin'] || '0000-01-01';
	const date_end   = date_range['end']   || '9999-12-31';
	return db.prepare(
...
function db_pull_query_summary(db, date_range) {
	if (date_range === undefined) date_range = {};
	const date_begin = date_range['begin'] || '0000-01-01';
	const date_end   = date_range['end']   || '9999-12-31';
	return db.prepare(
...
function test() {
	...
	console.log(db_pull_query_items(db, 3, {end: '2019-06-09'}));
	console.log(db_pull_query_IPs(db, 3));
	console.log(db_pull_query_summary(db));
